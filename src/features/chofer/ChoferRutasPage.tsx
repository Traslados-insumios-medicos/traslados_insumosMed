...
export function ChoferRutasPage() {
  ...
  const rutasFiltradas = rutas.filter((r) => {
    if (filtro === "activas")
      return r.estado === "PENDIENTE" || r.estado === "EN_CURSO";
    if (filtro === "completadas")
      return r.estado === "COMPLETADA" || r.estado === "CANCELADA";
    return true;
  });

  // Stats
  const totalGuias = rutas.flatMap((r) => r.guias).length;
  const entregadasTotal = rutas
    .flatMap((r) => r.guias)
    .filter((g) => g.estado === "ENTREGADO").length;
  const incidenciasTotal = rutas
    .flatMap((r) => r.guias)
    ...
    <div className="space-y-5 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4">
          ...
          <p
            className={`mt-1 text-2xl font-bold ${incidenciasTotal > 0 ? "text-amber-600" : "text-slate-900"}`}
          >
            {incidenciasTotal}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div>
            ...
            <input
              ...
              className="w-full min-w-0 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
            />
          </div>
                ...
                <div className="mt-4 space-y-1.5">
                  <div className="flex justify-between text-xs text-slate-500">
                    ...
                    <span className="font-semibold text-slate-700">
                      {entregadas + incidencias} / {total} guías
                    </span>
                  </div>
