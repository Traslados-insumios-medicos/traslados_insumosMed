...
function patchGuia(
  ...
  return {
    ...
    rutaActual: rutaActual
      ? {
          ...rutaActual,
          guias: updateGuias(rutaActual.guias),
          stops: updateStops(rutaActual.stops),
        }
      : null,
  };
}
...
export const useChoferStore = create<ChoferState>((set, get) => ({
  ...
  fetchMisRutas: async () => {
    ...
    try {
      ...
      const { data } = await api.get<{ data: RutaAPI[]; total: number }>(
        "/rutas",
      );
      set({ rutas: data.data, loading: false });
    } catch (e: unknown) {
      set({
        error: getApiErrorMessage(e, "Error al cargar rutas"),
        loading: false,
      });
    }
  },
  ...
  fetchRuta: async (id) => {
    ...
    try {
      ...
      set({
        error: getApiErrorMessage(e, "Error al cargar ruta"),
        loadingDetalle: false,
      });
    }
  },
  ...
  iniciarRuta: async (id) => {
    const { data } = await api.patch(`/rutas/${id}/estado`, {
      estado: "EN_CURSO",
    });
    set((s) => ({
      rutas: s.rutas.map((r) =>
        r.id === id ? { ...r, estado: "EN_CURSO" } : r,
      ),
      rutaActual:
        s.rutaActual?.id === id
          ? { ...s.rutaActual, estado: "EN_CURSO" }
          : s.rutaActual,
    }));
    return data;
  },
  ...
  finalizarRuta: async (id) => {
    ...
    set((s) => ({
      ...
      rutaActual:
        s.rutaActual?.id === id
          ? { ...s.rutaActual, estado: "COMPLETADA" }
          : s.rutaActual,
    }));
    return data;
  },
  ...
  guardarDetalle: async (guiaId, detalle) => {
    const { data } = await api.patch<GuiaAPI>(
  ...
  subirFotoGuia: async (guiaId, file) => {
    ...
    const addFoto = (guias: GuiaAPI[]) =>
      guias.map((g) =>
        g.id === guiaId ? { ...g, fotos: [...g.fotos, data] } : g,
      );
    ...
    set({
      rutas: rutas.map((r) => ({
        ...r,
        guias: addFoto(r.guias),
        stops: addFotoStops(r.stops),
      })),
      rutaActual: rutaActual
        ? {
            ...rutaActual,
            guias: addFoto(rutaActual.guias),
            stops: addFotoStops(rutaActual.stops),
          }
        : null,
    });
    return data;
  ...
  eliminarFoto: async (fotoId) => {
    ...
    const removeFoto = (fotos: FotoAPI[]) =>
      fotos.filter((f) => f.id !== fotoId);
