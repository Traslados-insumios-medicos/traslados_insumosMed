...
export function AdminChoferesPage() {
  ...
  const [deleteConfirmChofer, setDeleteConfirmChofer] = useState<Chofer | null>(
    null,
  );
  ...
  const fetchChoferes = useCallback(
    async (p: number, silent = false) => {
      ...
      try {
        ...
        setTotalActivosBackend(res.data.totalActivos ?? 0);
        setPage(p);
      } catch {
  ...
  const handleGenerateTempPassword = async (ch: Chofer) => {
    ...
    try {
      ...
      }>(`/auth/generate-temp-password/${userId}`);
  ...
  const openDetail = (id: string) => {
    ...
    api
      .get<ChoferDetalle>(`/usuarios/${id}`)
      .then((r) => setDetailChofer(r.data))
      .catch(() => {
        addToast("No se pudo cargar detalle", "error");
        setDetailId(null);
      })
  ...
  const handleToggleActivo = async (id: string) => {
    ...
    setChoferes((prev) =>
      prev.map((ch) => (ch.id === id ? { ...ch, activo: !ch.activo } : ch)),
    );
    try {
      const res = await api.patch<Chofer>(`/usuarios/${id}/toggle-activo`);
      setChoferes((prev) =>
        prev.map((ch) =>
          ch.id === id ? { ...ch, activo: res.data.activo } : ch,
        ),
      );
    } catch {
      setChoferes((prev) =>
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        setChoferes((prev) =>
          prev.map((ch) => (ch.id === editingId ? res.data : ch)),
        );
        ...
        const res = await api.post<{
          usuario: Chofer;
          passwordTemporal: string;
        }>("/auth/register", { nombre, email, cedula, celular, rol: "CHOFER" });
        setPasswordModal({
          choferNombre: nombre,
          password: res.data.passwordTemporal,
        });
      ...
      addToast(
        status === 409
          ? "Ya existe un chofer con ese email o celular"
          : "Error al guardar",
        "error",
      );
    } finally {
  ...
  const trunc = (str: string, max = 25) =>
    str.length > max ? str.slice(0, max) + "..." : str;

  // La búsqueda va al backend — choferesFiltrados es simplemente los datos de la página actual
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-6">
          <div>
            ...
            <input
              className={`w-full rounded-lg border bg-white px-3 py-2.5 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/20 ${nombreError ? "border-red-400 focus:border-red-400" : "border-slate-300 focus:border-primary"}`}
              value={nombre}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                className={`w-full rounded-lg border bg-white px-3 py-2.5 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/20 ${cedulaError ? "border-red-400 focus:border-red-400" : "border-slate-300 focus:border-primary"}`}
                value={cedula}
              ...
              />
              {cedulaError && (
                <p className="mt-1 text-xs text-red-500">{cedulaError}</p>
              )}
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-500">
              ...
              <input
                className={`w-full rounded-lg border bg-white px-3 py-2.5 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/20 ${celularError ? "border-red-400 focus:border-red-400" : "border-slate-300 focus:border-primary"}`}
                value={celular}
              ...
              />
              {celularError && (
                <p className="mt-1 text-xs text-red-500">{celularError}</p>
              )}
            </div>
          </div>
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span
                className={`text-[10px] ${email.length > 120 ? "text-amber-500" : "text-slate-400"}`}
              >
                {email.length}/150
              </span>
            </div>
            <input
              type="email"
              className={`w-full rounded-lg border bg-white px-3 py-2.5 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary/20 ${emailError ? "border-red-400 focus:border-red-400" : "border-slate-300 focus:border-primary"}`}
              value={email}
          ...
          {editingId && (
            <div className="border-t border-slate-100 pt-4">
              ...
              <p className="mt-2 text-xs text-slate-500 text-center">
                Genera una nueva contraseña temporal si el chofer olvidó su
                contraseña
              </p>
            </div>
          )}
          ...
          <>
            ...
            <div className="space-y-4 p-6">
              ...
              <div className="rounded-xl bg-amber-50 p-4">
                ...
                <div className="flex items-center gap-3">
                  <code className="flex-1 rounded-lg bg-white px-3 py-2 font-mono text-sm font-bold text-slate-900 shadow-sm">
              ...
              <p className="text-xs text-slate-400">
                No se volverá a mostrar esta contraseña.
              </p>
              ...
              <dl className="space-y-3 text-sm">
                <div>
                  ...
                  <dd className="break-words text-slate-900">
                    {detailChofer.nombre.length > 50
                      ? detailChofer.nombre.slice(0, 47) + "..."
                      : detailChofer.nombre}
                  </dd>
                </div>
                <div>
                  ...
                  <dd className="break-words text-slate-900">
                    {detailChofer.email.length > 50
                      ? detailChofer.email.slice(0, 47) + "..."
                      : detailChofer.email}
                  </dd>
                </div>
                <div>
                ...
                <div>
                  ...
                  <dd className="text-slate-900">
                    {detailChofer.activo ? "Activo" : "Inactivo"}
                  </dd>
                </div>
              </dl>
          ...
          <>
            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4">
              ...
              <button
                type="button"
                onClick={() =>
                  !deleteChoferSubmitting && setDeleteConfirmChofer(null)
                }
            ...
            <div className="space-y-4 p-6">
              <p className="text-sm text-slate-600">
                ...
                ? En la base de datos se eliminan{" "}
                <span className="font-semibold">todas las rutas</span> donde
                figura como chofer (paradas, guías, fotos y seguimiento) y
                después el chofer. No queda rastro operativo de esas rutas.
              </p>
              ...
              <div className="flex flex-wrap justify-end gap-3 border-t border-slate-100 pt-4">
                <button
                  type="button"
                  disabled={deleteChoferSubmitting}
                  onClick={() => setDeleteConfirmChofer(null)}
                ...
                <button
                  type="button"
                  disabled={deleteChoferSubmitting}
                  onClick={() => void executeDeleteChofer()}
