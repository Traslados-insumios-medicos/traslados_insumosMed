...
export function AdminClientesPage() {
  ...
  const [passwordModal, setPasswordModal] = useState<PasswordModalData | null>(
    null,
  );
  ...
  const handleNombreChange = (val: string) => {
    ...
    setNombreError(
      !/^[a-zA-ZáéíóúÁÉÍÓÚüÜñÑ\s]+$/.test(val)
        ? "El nombre solo debe contener letras, tildes y ñ"
        : "",
    );
  };
  ...
  const handleTelefonoChange = (val: string) => {
    ...
    setTelefonoError(
      val.length !== 10 ? "El teléfono debe tener exactamente 10 dígitos" : "",
    );
  };
  ...
  const handleEmailContactoChange = (val: string) => {
    ...
    if (!val) {
      ...
      setEmailContactoError(
        "El email debe contener @, dominio y extensión válida (ej. usuario@empresa.com)",
      );
    } else {
      setEmailContactoError("");
    }
  ...
  useEffect(() => {
    api
      .get<PaginatedResponse>("/clientes?tipo=PRINCIPAL&limit=100")
      .then((r) =>
        setPrincipales(
          r.data.data.map((c) => ({ id: c.id, nombre: c.nombre })),
        ),
      )
      .catch(() => {});
  }, []);
  ...
  const resetForm = () => {
    ...
    setEmailContacto("");
    setTipo("SECUNDARIO");
    setTipoOriginal(null);
    setClientePrincipalId("");
    setUsarMismoEmail(false);
    setUsuarioNombre("");
    setUsuarioEmail("");
    setShowModal(false);
    setNombreError("");
  ...
  const handleGenerateTempPassword = async (c: Cliente) => {
    ...
    try {
      ...
      }>(`/auth/generate-temp-password/${usuarioId}`);
  ...
  const handleToggleActivo = async (id: string) => {
    ...
    try {
      ...
      setClientes((prev) =>
        prev.map((c) => (c.id === id ? { ...c, activo: res.data.activo } : c)),
      );
    } catch {
      setClientes((prev) =>
        prev.map((c) => (c.id === id ? { ...c, activo: !c.activo } : c)),
  ...
  const executeDeleteCliente = async () => {
    ...
    try {
      ...
      addToast(
        c.tipo === "PRINCIPAL"
          ? "Cliente y datos vinculados eliminados"
          : "Cliente eliminado",
        "success",
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      (editingId && tipoOriginal === "SECUNDARIO" && tipo === "PRINCIPAL") ||
    ...
    try {
      if (editingId) {
        const res = await api.put<Cliente>(`/clientes/${editingId}`, {
          ...
          tipo,
          clientePrincipalId:
            tipo === "SECUNDARIO" ? clientePrincipalId || undefined : undefined,
        });
        setClientes((prev) =>
        ...
        if (
          ...
          try {
            ...
            setPasswordModal({
              ...
              email: usuarioEmail,
              password: userRes.data.passwordTemporal,
            });
          } catch (userErr: unknown) {
            const userMessage = (
              userErr as { response?: { data?: { message?: string } } }
            )?.response?.data?.message;
            addToast(
              userMessage || "Error al crear usuario de acceso",
              "error",
            );
          }
        }
  ...
  const actionIconClass =
    "flex items-center justify-center rounded p-1.5 text-slate-400 transition-colors hover:text-primary";
  const rowActions = (c: Cliente) => (
    <>
      ...
      <button
        ...
        onClick={(e) => {
          e.stopPropagation();
          openDeleteClienteModal(c);
        }}
        className={`${actionIconClass} hover:text-red-600`}
  ...
  return (
    <div className="space-y-4">
      ...
      <ModalMotion
        ...
        panelClassName="w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-2xl bg-white shadow-2xl"
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-5 p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {(["PRINCIPAL", "SECUNDARIO"] as TipoCliente[]).map((t) => (
                <label
                  key={t}
                  className={[
                    "flex cursor-pointer items-center gap-3 rounded-xl border-2 px-3 py-3 transition-all",
                    tipo === t
                ...
                >
                  ...
                  <div
                    className={[
                      ...
                      tipo === t ? "bg-primary/10" : "bg-slate-100",
                    ].join(" ")}
                  >
                    <span
                      className={[
                        "material-symbols-outlined text-[18px]",
                        tipo === t ? "text-primary" : "text-slate-400",
                      ].join(" ")}
                    >
                  ...
                  <div className="min-w-0 flex-1">
                    <p
                      className={[
                        ...
                        tipo === t ? "text-primary" : "text-slate-800",
                      ].join(" ")}
                    >
                  ...
                  {tipo === t && (
                    <span className="ml-auto shrink-0 material-symbols-outlined text-base text-primary">
                      check_circle
                    </span>
                  )}
                </label>
            ...
            <div className="space-y-2">
              ...
              <SearchableSelect
                options={[
                  ...
                  ...principales.map((p) => ({ value: p.id, label: p.nombre })),
                ]}
                value={clientePrincipalId}
                onChange={(value) => setClientePrincipalId(value)}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-[10px] ${nombre.length > 80 ? "text-amber-500" : "text-slate-400"}`}
                >
                  {nombre.length}/100
                </span>
              </div>
              <input
                className={`w-full rounded-lg border bg-white px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary ${nombreError ? "border-red-400" : "border-slate-300"}`}
                value={nombre}
            ...
            <div className="space-y-2">
              ...
              <input
                className={`w-full rounded-lg border bg-white px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary ${rucError ? "border-red-400" : "border-slate-300"}`}
                value={ruc}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              ...
              <input
                className={`w-full rounded-lg border bg-white px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary ${telefonoError ? "border-red-400" : "border-slate-300"}`}
                value={telefono}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-[10px] ${emailContacto.length > 120 ? "text-amber-500" : "text-slate-400"}`}
                >
                  {emailContacto.length}/150
                </span>
              </div>
              <input
                type="email"
                className={`w-full rounded-lg border bg-white px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary ${emailContactoError ? "border-red-400" : "border-slate-300"}`}
                value={emailContacto}
                onChange={(e) => handleEmailContactoChange(e.target.value)}
                onBlur={() => {
              ...
              />
              {emailContactoError && (
                <p className="text-xs text-red-500">{emailContactoError}</p>
              )}
            </div>
          </div>
          {((editingId &&
            ...
            <div className="rounded-xl border border-slate-200 p-4 space-y-4">
              <div className="text-sm font-semibold text-slate-700 border-b border-slate-100 pb-2">
                Usuario de acceso
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <input
                    className={`w-full rounded-lg border bg-white px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary ${usuarioNombreError ? "border-red-400" : "border-slate-300"}`}
                    value={usuarioNombre}
                    onChange={(e) => handleUsuarioNombreChange(e.target.value)}
                    onBlur={() => {
                  ...
                  />
                  {usuarioNombreError && (
                    <p className="text-xs text-red-500">{usuarioNombreError}</p>
                  )}
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-[10px] ${usuarioEmail.length > 120 ? "text-amber-500" : "text-slate-400"}`}
                    >
                      {usuarioEmail.length}/150
                    </span>
                  </div>
                  <input
                    type="email"
                    className={`w-full rounded-lg border bg-white px-3 py-2.5 text-sm focus:ring-2 focus:ring-primary ${usuarioEmailError ? "border-red-400" : "border-slate-300"}`}
                    value={usuarioEmail}
                    onChange={(e) => handleUsuarioEmailChange(e.target.value)}
                    onBlur={() => {
                    ...
                    required
                    maxLength={150}
                    disabled={usarMismoEmail}
                  />
                  {usuarioEmailError && (
                    <p className="text-xs text-red-500">{usuarioEmailError}</p>
                  )}
                </div>
              </div>
              <label className="flex cursor-pointer items-center gap-2">
                ...
                <span className="text-xs text-slate-600">
                  Usar el mismo correo del cliente
                </span>
              </label>
            </div>
          )}
          {editingId && tipo === "PRINCIPAL" && (
            <div className="border-t border-slate-200 pt-4">
              ...
              <p className="mt-2 text-xs text-slate-500 text-center">
                Genera una nueva contraseña temporal si el cliente olvidó su
                contraseña
              </p>
            </div>
          )}
            ...
            <div className="space-y-4 p-6">
              ...
              <div className="rounded-xl bg-amber-50 p-4">
                ...
                <div className="flex items-center gap-3">
                  <code className="flex-1 rounded-lg bg-white px-3 py-2.5 font-mono font-bold text-slate-900 shadow-sm">
          ...
          <>
            <div className="flex items-center justify-between border-b border-slate-200 p-6">
              ...
              <button
                type="button"
                onClick={() =>
                  !deleteClienteSubmitting && setDeleteConfirmCliente(null)
                }
            ...
            <div className="space-y-4 p-6">
              <p className="text-sm text-slate-600">
                ...
                <span className="font-semibold text-slate-900">
                  {deleteConfirmCliente.nombre}
                </span>
                ? Se eliminan en la base todas sus guías y paradas, el usuario
                de panel cliente si existe y{" "}
                <span className="font-semibold">
                  cualquier ruta que quede vacía
                </span>{" "}
                (sin paradas) con todo lo vinculado. Rutas compartidas con otros
                clientes siguen si los demás puntos permanecen.
              </p>
              {deleteConfirmCliente.tipo === "PRINCIPAL" &&
                (deleteConfirmCliente.clientesSecundarios?.length ?? 0) > 0 && (
                  <p className="text-xs font-medium text-amber-700">
                    Es principal: también se eliminarán todos sus clientes
                    secundarios (
                    {deleteConfirmCliente.clientesSecundarios!.length}) y sus
                    datos vinculados.
                  </p>
                )}
              <p className="text-xs text-slate-500">
                Para solo desactivar el acceso, use el interruptor en la columna
                Estado.
              </p>
              <div className="flex flex-wrap justify-end gap-3 border-t border-slate-200 pt-4">
                <button
                  type="button"
                  disabled={deleteClienteSubmitting}
                ...
                <button
                  type="button"
                  disabled={deleteClienteSubmitting}
                  onClick={() => void executeDeleteCliente()}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="mt-2 text-sm text-slate-600">
                Al cambiar este cliente de{" "}
                <span className="font-semibold">Principal</span> a{" "}
                <span className="font-semibold">Secundario</span>, se eliminará
                el usuario de acceso asociado y perderá acceso al sistema.
              </p>
              ...
              onClick={async () => {
                ...
                try {
                  await api.put<Cliente>(`/clientes/${editingId}`, {
                    ...
                    clientePrincipalId:
                      tipo === "SECUNDARIO"
                        ? clientePrincipalId || undefined
                        : undefined,
                  });
                  addToast(
                    "Cliente actualizado y usuario de acceso eliminado",
                    "success",
                  );
                  await fetchClientes(page);
                  resetForm();
              ...
              <dl className="grid gap-3 sm:grid-cols-2">
                <div>
                  ...
                  <dd className="text-sm text-slate-900">
                    {detailCliente.nombre}
                  </dd>
                </div>
                <div>
                  <dt className="text-xs font-semibold text-slate-400">RUC</dt>
                  <dd className="text-sm text-slate-900">
                    {detailCliente.ruc}
                  </dd>
                </div>
                <div>
                  <dt className="text-xs font-semibold text-slate-400">Tipo</dt>
                  <dd className="text-sm text-slate-900">
                    {detailCliente.tipo}
                  </dd>
                </div>
                ...
                <div>
                  ...
                  <dd className="text-sm text-slate-900">
                    {detailCliente.telefonoContacto ?? "—"}
                  </dd>
                </div>
                <div>
                  <dt className="text-xs font-semibold text-slate-400">
                ...
                clientesFiltrados.map((c) => (
                  <div key={c.id} className="px-4 py-3.5">
                    <div className="flex items-center gap-3 mb-2.5">
                      <div
                        className={`flex size-9 shrink-0 items-center justify-center rounded-lg ${c.tipo === "PRINCIPAL" ? "bg-primary/10" : "bg-slate-100"}`}
                      >
                        <span
                          className={`material-symbols-outlined text-[17px] ${c.tipo === "PRINCIPAL" ? "text-primary" : "text-slate-400"}`}
                        >
                          {c.tipo === "PRINCIPAL"
                            ? "corporate_fare"
                            : "location_on"}
                  ...
                  <th className="px-4 py-2.5">
                    <div className="relative overflow-hidden">
                      <input
                        ...
                        onFocus={(e) => {
                          if (e.target.value === "Cliente Principal")
                            setFiltroPrincipal("");
                        }}
                        onBlur={(e) => {
                          if (e.target.value === "")
                            setFiltroPrincipal("Cliente Principal");
                        }}
                      ...
                      <td className="px-4 py-3">
                        {c.tipo === "PRINCIPAL" ? (
                          ...
                          <span className="text-xs text-slate-700">
                            {c.clientePrincipal?.nombre || "Sin asignar"}
                          </span>
                        )}
                      </td>
                      <td className="px-4 py-3">
