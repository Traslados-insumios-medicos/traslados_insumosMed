...
export function AdminRutaTiempoRealPage() {
  ...
  const entregadas = ruta.stops.reduce(
    (a, s) => a + s.guias.filter((g) => g.estado === "ENTREGADO").length,
    0,
  );
  const incidencias = ruta.stops.reduce(
    (a, s) => a + s.guias.filter((g) => g.estado === "INCIDENCIA").length,
    0,
  );
