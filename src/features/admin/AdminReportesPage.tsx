...
export function AdminReportesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      if (tab === "cliente") {
        ...
        if (fechaDesde) params.set("desde", fechaDesde);
        if (fechaHasta) params.set("hasta", fechaHasta);
        const res = await api.get<ResumenChofer[]>(
          `/reportes/choferes?${params}`,
        );
  ...
  const buildFilterInfo = () => {
    ...
    if (tipoCliente) {
      filters.push(
        `Tipo de cliente: ${tipoCliente === "PRINCIPAL" ? "Principales" : "Secundarios"}`,
      );
    }
    if (clienteId) {
  ...
  const handleExportClienteExcel = () => {
    const detalleRows = dataCliente.flatMap((cliente) =>
      cliente.guias.map((g) => [
        ...
        g.horaLlegada ?? "—",
        g.horaSalida ?? "—",
        parseMultiFieldSuffix(g.temperatura, "°C"),
        g.observaciones ?? "—",
  ...
  const buildFechasRows = () =>
    dataFechas.map((g) => ({
      ...
      Observaciones: g.observaciones ?? "—",
      Incidencias: formatNovedades(g.novedades ?? []),
      "Fotos (URLs)": formatFotos(
        (g.fotos ?? []).filter((f) => f.tipo === "GUIA"),
      ),
  ...
  const buildGuiaRows = () =>
    dataGuia.map((g) => ({
      ...
      "Lugar destino": g.ruta?.lugarDestino?.trim() || "—",
      "Ruta ID": g.ruta?.id ?? "—",
      "Fecha ruta": g.ruta?.fecha
        ? new Date(g.ruta.fecha).toLocaleDateString("es-ES")
        : "—",
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">
          Filtros y reportes por cliente, fechas y chofer.
        </p>
      </div>
          ...
          <>
                ...
                <div className="divide-y divide-slate-100">
                  {dataChoferPaginada.map((ch) => {
                    ...
                    const entregadas = ch.rutas.reduce(
                      (a, r) =>
                        a +
                        r.guias.filter((g) => g.estado === "ENTREGADO").length,
                      0,
                    );
                    return (
                      <div key={ch.choferId}>
                        <button
                          type="button"
                          onClick={() =>
                            setChoferExpandidoId(expandido ? null : ch.choferId)
                          }
            ...
            {tab === "guia" && (
              <div className="p-4">
                <p className="mb-4 text-sm text-slate-500">
                  Total de guías:{" "}
                  <strong className="text-slate-900">{dataGuia.length}</strong>
                  {(fechaDesde || fechaHasta) && (
                    <span className="ml-2 text-xs">
                      ({fechaDesde && `desde ${fechaDesde}`}{" "}
                      {fechaHasta && `hasta ${fechaHasta}`})
                ...
                {totalPagesGuia > 1 && (
                  <div className="flex items-center justify-between border-t border-slate-100 mt-4 pt-3 text-sm">
                    <p className="text-slate-500">
                      {dataGuia.length} guía{dataGuia.length !== 1 ? "s" : ""}
                    </p>
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
