...
export function AdminSeguimientoPage() {
  ...
  const fetchRutas = useCallback(async () => {
    try {
      const res = await api.get<{ data: RutaActiva[] }>(
        "/rutas?estado=EN_CURSO&limit=50",
      );
  ...
  useEffect(() => {
    fetchRutas();
    const i = setInterval(fetchRutas, 30000);
    return () => clearInterval(i);
  }, [fetchRutas]);

  const abrirDetalle = async (rutaId: string) => {
    setDetalleRutaId(rutaId);
    setDetalleLoading(true);
    setDetalleRuta(null);
    try {
      const res = await api.get<RutaDetalle>(`/rutas/${rutaId}`);
  ...
  useEffect(() => {
    ...
    posiciones.forEach((pos, rutaId) => {
      ...
      const nombre =
        pos.choferNombre ||
        rutasActivas.find((r) => r.id === rutaId)?.chofer.nombre ||
        "Chofer";
    ...
    markersRef.current.forEach((marker, rutaId) => {
      if (!posiciones.has(rutaId)) {
        marker.remove();
        markersRef.current.delete(rutaId);
      }
    });
        ...
        {loading ? (
          ...
          rutasActivas.map((ruta, i) => {
            ...
            return (
              <div
                key={ruta.id}
                className={`rounded-xl border transition-all ${isSelected ? "border-primary bg-primary/5 shadow-md" : "border-slate-200 bg-white"}`}
              >
                <button
                  type="button"
                  onClick={() => setSelectedRutaId(isSelected ? null : ruta.id)}
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-400">{ruta.fecha}</p>
                      </div>
                    </div>
                  ...
                  <div className="mt-3 space-y-1.5">
                    ...
                    <p className="text-[11px] text-slate-400">
                      {
                        ruta.guias.filter((g) => g.estado === "ENTREGADO")
                          .length
                      }
                      /{ruta.guias.length} guias entregadas
                      {ruta.guias.some((g) => g.estado === "INCIDENCIA") && (
                        <span className="ml-2 text-amber-600">incidencia</span>
                      )}
                    </p>
                  </div>
                </button>
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-700">
              Mapa global de choferes
            </span>
          </div>
          <span className="text-xs text-slate-400">
            {posiciones.size > 0
              ? `${posiciones.size} chofer${posiciones.size > 1 ? "es" : ""} con GPS activo`
              : "Esperando senal GPS"}
          </span>
        </div>
        ...
        <div className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col border-l border-slate-200 bg-white shadow-2xl">
          <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
            <div>
              ...
              {detalleRuta && (
                <p className="text-xs text-slate-500">
                  {detalleRuta.chofer.nombre} · {detalleRuta.fecha}
                </p>
              )}
            </div>
            <button
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    {
                      label: "Total",
                      value: detalleRuta.guias.length,
                      cls: "text-slate-700",
                    },
                    {
                      label: "Entregadas",
                      value: detalleRuta.guias.filter(
