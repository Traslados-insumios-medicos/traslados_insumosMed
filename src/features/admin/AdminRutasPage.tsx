    ...
    <div className="space-y-6">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl bg-white shadow-2xl dark:bg-slate-800">
            ...
            <div className="space-y-5 p-5">
              <div>
                <label className="mb-1.5 block text-sm font-semibold text-slate-700 dark:text-slate-300">Chofer</label>
                <select
                  value={choferId}
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Paradas</label>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {rutas.map((ruta) => {
            ...
            return (
              <div key={ruta.id} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
                <button
                  type="button"
                ...
                >
                  <div className="flex min-w-0 flex-wrap items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-slate-900 dark:text-white">Ruta #{ruta.id.slice(-6)}</p>
                      <p className="text-sm text-slate-500 dark:text-slate-400">
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                      <div>
                        ...
                        <ul className="space-y-3">
                          {ruta.stops.map((stop) => (
                            <li key={stop.id} className="rounded-lg border border-slate-200 bg-slate-50 p-3 dark:border-slate-600 dark:bg-slate-700/30">
                              <p className="text-xs font-semibold uppercase tracking-wider text-primary">Parada #{stop.orden}</p>
                              <p className="mt-1 text-sm font-medium text-slate-900 dark:text-white">{stop.direccion}</p>
                              ...
                              {stop.guias.length > 0 && (
                                <ul className="mt-2 space-y-1 border-t border-slate-200 pt-2 dark:border-slate-600">
                                  {stop.guias.map((g) => (
                                    <li key={g.id} className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-300">
                      ...
                      <div>
                        ...
                        {ruta.fotos.length === 0 ? (
                          ...
                          <div className="flex flex-wrap gap-3">
                            {ruta.fotos.map((f) => (
                              <div key={f.id} className="overflow-hidden rounded-lg border border-slate-200 dark:border-slate-600">
                                <img src={f.urlPreview} alt="Hoja de ruta" className="h-40 w-auto max-w-full object-cover" />
                                <p className="border-t border-slate-100 bg-slate-50 px-2 py-1.5 text-[10px] text-slate-500 dark:border-slate-600 dark:bg-slate-700/50">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-sm">
          <p className="text-slate-500 dark:text-slate-400">{total} ruta{total !== 1 ? 's' : ''} en total</p>
