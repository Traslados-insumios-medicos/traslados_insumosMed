...
export function ClienteEnvioDetallePage() {
      ...
      (p: { rutaId: string; seguimientoChofer: string }) => {
        ...
        setGuia((prev) =>
          prev?.ruta
            ? {
                ...prev,
                ruta: { ...prev.ruta, seguimientoChofer: p.seguimientoChofer },
              }
            : prev,
        );
        setSeguimientoActualizadoAt(new Date().toISOString());
    ...
    return (
      <div className="rounded-xl border border-slate-200 bg-white p-8">
        <p className="text-sm text-slate-500">
          Guía no encontrada o sin permiso.
        </p>
        <Link
          to="/cliente/envios"
  ...
  const statusSteps = [
    { key: "created", label: "Pedido recibido", done: true, active: false },
    {
      key: "transit",
      label: "En camino",
      done: guia.estado !== "PENDIENTE",
      active: guia.estado === "PENDIENTE",
    },
    {
      key: "delivery",
      label: "En entrega",
  ...
  const handleExportPDF = () => {
    ...
    exportToPDF(
      ...
      [
        ...
        [
          "Novedades",
          novedadesGuia.map((n) => n.descripcion).join(" | ") || "—",
        ],
        ["Fecha creación", new Date(guia.createdAt).toLocaleString("es-ES")],
      ],
      `reporte-guia-${guia.numeroGuia}`,
      ...
      [
        {
          ...
          fields: [
            ...
            {
              label: "Novedades",
              value: novedadesGuia.map((n) => n.descripcion).join(" | ") || "—",
            },
          ],
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mb-4 text-xs text-slate-500">
              Esta línea de tiempo se actualiza automáticamente con los cambios
              reportados por el chofer.
            </p>
                ...
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                  {guia.receptorNombre && (
                    <div>
                      ...
                      {guia.receptorNombre.split("|").map((n, i) => (
                        <p
                          key={i}
                  ...
                  {guia.temperatura && (
                    <div>
                      ...
                      {guia.temperatura.split("|").map((t, i) => (
                        <p
                          key={i}
                          className="mt-1 text-sm font-medium text-slate-900"
                        >
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            {novedadesGuia.length === 0 ? (
              <p className="text-sm text-slate-500">
                Sin novedades registradas para esta guía.
              </p>
            ) : (
                ...
                {novedadesGuia.map((n) => (
                  ...
                  >
                    <p className="font-semibold text-slate-900">
                      {tipoNovedadLabel[n.tipo] ?? n.tipo}
                    </p>
                    <p className="text-xs text-slate-500">
        ...
        <div className="flex flex-col gap-6">
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            {fotosGuia.length === 0 ? (
              <p className="text-sm text-slate-500">
                No hay fotos registradas para esta guía.
              </p>
            ) : (
            ...
            <p className="mt-4 text-[10px] italic text-slate-400">
              Haga clic en la imagen para verla en tamaño completo o en el ícono
              de descarga para guardarla.
            </p>
          </div>
        </div>
      </div>
