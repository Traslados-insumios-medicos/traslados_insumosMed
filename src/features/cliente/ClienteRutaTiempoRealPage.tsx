      ...
      {!guiaActiva || !ruta ? (
        ...
        <>
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800 shadow-sm">
            ...
            <p className="border-t border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-500 dark:text-slate-400">
              Posición del camión simulada cada 4 s. Mapa: OpenStreetMap · Leaflet.
            </p>
          </div>

          <div className="rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800 p-4">
            <p className="text-sm font-semibold text-slate-900 dark:text-white">
              Guía {guiaActiva.numeroGuia}
            </p>
            <p className="text-xs text-slate-500 dark:text-slate-400">{guiaActiva.descripcion}</p>
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Ruta {ruta.id}</p>
          </div>
        </>
      )}
    </div>
