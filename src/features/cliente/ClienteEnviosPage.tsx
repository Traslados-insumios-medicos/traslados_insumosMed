...
const estadoClass: Record<string, string> = {
  ...
  INCIDENCIA: 'bg-rose-100 text-rose-800 dark:bg-rose-900/40 dark:text-rose-300',
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6 sm:grid-cols-2 md:grid-cols-3">
        ...
        <div className="flex flex-col gap-2 rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800 p-4 shadow-sm sm:p-6">
          <div className="flex items-start justify-between">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Entregados hoy</p>
            <span className="rounded-lg bg-emerald-100 p-2 text-emerald-600">
              <span className="material-symbols-outlined">task_alt</span>
            </span>
          </div>
          <p className="text-3xl font-bold leading-tight text-slate-900 dark:text-white">{entregadosHoy}</p>
          <div className="flex items-center gap-1 text-sm font-semibold text-emerald-600">
            <span className="material-symbols-outlined text-xs">trending_up</span>
            <span>Al día</span>
          </div>
        ...
        <div className="flex flex-col gap-2 rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800 p-4 shadow-sm sm:p-6">
          <div className="flex items-start justify-between">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Con incidencia</p>
            <span className="rounded-lg bg-amber-100 p-2 text-amber-600">
              <span className="material-symbols-outlined">warning</span>
            </span>
          </div>
          <p className="text-3xl font-bold leading-tight text-slate-900 dark:text-white">{conIncidencia}</p>
                ...
                guiasFiltradas.map((g) => {
                  ...
                  return (
                    <tr key={g.id} className="transition-colors hover:bg-slate-50 dark:hover:bg-slate-700/50">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-700 dark:text-slate-200">{g.descripcion}</td>
