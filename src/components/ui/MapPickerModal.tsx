...
export function MapPickerModal({
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      if (initialCoords) {
        ...
        fetch(
          `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(initialAddress)}.json?access_token=${mapboxgl.accessToken}&limit=1&language=es&country=EC`,
        )
          .then((r) => r.json())
          .then((data) => {
            const feature = data.features?.[0];
  ...
  const handleSearch = (val: string) => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await fetch(
          `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(val)}.json?access_token=${mapboxgl.accessToken}&autocomplete=true&limit=5&language=es&country=EC`,
        );
        const data = await res.json();
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4">
      ...
      >
        ...
        <div className="relative border-b border-slate-100 px-4 py-3">
          <span className="material-symbols-outlined absolute left-7 top-1/2 -translate-y-1/2 text-base text-slate-400">
            search
          </span>
          <input
            type="text"
          ...
          {openSug && suggestions.length > 0 && (
            <div className="absolute left-4 right-4 top-full z-10 mt-1 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-lg">
              {suggestions.map((s) => (
                ...
                >
                  <span className="material-symbols-outlined mt-0.5 shrink-0 text-sm text-slate-400">
                    location_on
                  </span>
        ...
        <div className="border-t border-slate-200 px-5 py-3">
          {address ? (
            <p className="mb-2 flex items-start gap-1.5 text-xs text-slate-600">
              <span className="material-symbols-outlined mt-0.5 shrink-0 text-sm text-emerald-500">
                check_circle
              </span>
