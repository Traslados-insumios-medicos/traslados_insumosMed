...
export function MainLayout() {
  ...
  return (
    <div
      className="flex overflow-hidden bg-bg font-body text-slate-800 antialiased"
      style={{ height: "100dvh" }}
    >
      {/* Mobile overlay */}
      {sidebarOpen && (
      ...
      <aside
        style={{ height: "100dvh" }}
        className={`fixed top-0 left-0 z-50 flex w-60 flex-col bg-white shadow-[1px_0_0_0_#e2e8f0] transition-transform duration-200 md:relative md:h-full md:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {/* Brand */}
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="font-display text-sm font-semibold text-slate-800 truncate">
                ...
                {role === "CHOFER" && (
                  <>
                    <span className="hidden sm:inline">Panel del Chofer</span>
                    <span className="sm:hidden">Chofer</span>
                  </>
                )}
                {role === "CLIENTE" && (
                  <>
