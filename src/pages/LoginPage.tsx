...
export function LoginPage() {
    ...
    mm.add(
      ...
      (context) => {
        ...
        if (leftPanelRef.current) {
          const leftEls =
            leftPanelRef.current.querySelectorAll<HTMLElement>(".login-reveal");
          tl.from(
            leftEls,
            {
              autoAlpha: rm ? 1 : 0,
  ...
  const [inactiveMsg, setInactiveMsg] = useState(
    "Su acceso está inactivo. Contacte al administrador de la empresa.",
  );
  const [showSessionExpiredModal, setShowSessionExpiredModal] = useState(false);
  const [sessionExpiredMsg, setSessionExpiredMsg] = useState(
    "Su sesión ha expirado. Otra sesión ha sido iniciada con esta cuenta.",
  );
  ...
  useEffect(() => {
    ...
    const sessionExpiredNotice = sessionStorage.getItem(
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const { mustChangePassword } = await loginWithCredentials(
        email,
        password,
      );
    ...
    <div className="flex min-h-screen font-body">
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <img
            src={logo}
            alt=""
            className="w-[420px] h-auto brightness-0 invert opacity-[0.15]"
          />
        </div>
        ...
        <div className="login-reveal relative z-10 space-y-6">
          <div>
            ...
            <p className="mt-2 text-lg font-light text-blue-200">
        ...
        <p className="login-reveal absolute bottom-10 left-12 z-10 text-xs text-blue-300">
          © 2025 LOGISTRANS S.A. Todos los derechos reservados.
        </p>
      </div>
        ...
        <div className="w-full max-w-[380px]">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[20px] text-slate-400 flex items-center">
                  lock
                </span>
                <input
      ...
      >
        ...
        <div className="space-y-4 px-6 py-5">
          <p className="text-sm text-slate-600">{inactiveMsg}</p>
          <p className="text-xs text-slate-400">
            Cuando el administrador reactive su usuario, podrá volver a ingresar
            normalmente.
          </p>
      ...
      >
        ...
        <div className="space-y-4 px-6 py-5">
          <p className="text-sm text-slate-600">{sessionExpiredMsg}</p>
          <p className="text-xs text-slate-400">
            Solo puede haber una sesión activa por cuenta. Si necesita acceder
            desde múltiples dispositivos, contacte al administrador.
          </p>
          <div className="flex justify-end">
            <button
              type="button"
